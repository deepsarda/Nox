        ...
        return when {
            ...
            type.isArray -> {
                val list =
                    when (value) {
                        is String ->
                            parseJson(value) as? List<*>
                                ?: throw typeError(type, value, "Expected a JSON array")
                        is List<*> -> value
                        else -> throw typeError(type, value)
                    }
            ...
            type.isStructType() -> {
                ...
                val map =
                    when (value) {
                        is String ->
                            parseJson(value) as? Map<*, *>
                                ?: throw typeError(type, value, "Expected a JSON object")
                        is Map<*, *> -> value
                        else -> throw typeError(type, value)
                    }

                val structDef =
                    program?.typesByName?.get(type.name)
                ...
                for (field in structDef.fields) {
                    ...
                    if (fieldValue == null) {
                        throw NoxException(
                            NoxError.TypeError,
                            "Missing required field '${field.name}' of type '${field.type}' for struct '${type.name}'",
                            0,
                        )
                    }
