...
class ExpressionEmitter(
    ...
    private fun emitFuncCall(
        ...
        // Look up TypedFuncDef for typed default values
        val typedFuncDef =
            ctx.program.functionsByName[funcDef.name]
                ?: ctx.modules.firstNotNullOfOrNull { it.program.functionsByName[funcDef.name] }
        val typedParams = typedFuncDef?.params

        emitArgs(expr.args, funcDef.params, typedParams, primArgStart, refArgStart, line, pOffset, rOffset)
    ...
    private fun emitArgs(
        ...
        for (i in params.indices) {
            ...
            if (param.isVarargs) {
                ...
                // Emit default value from the typed parameter definition
                val typedDefault = typedParams?.getOrNull(i)?.defaultValue
                if (typedDefault != null) {
                    emitExpr(typedDefault, argReg, line)
                }
            }
        }
    }
