...
class ExpressionResolver(
        ...
        val typedParts =
            expr.parts.map { part ->
                when (part) {
                    ...
                    is RawTemplatePart.Interpolation ->
    ...
    private fun resolveArrayLiteral(
        ...
        for (i in 1 until expr.elements.size) {
            ...
            errors.report(
                expr.elements[i].loc,
                "Type mismatch: Array element ${i + 1} has type '$elemType' which is not compatible with inferred array element type '$elementType'",
                suggestion = "Ensure all elements have the same type, or declare the variable with an explicit type",
            )
            typedElements.add(typedElem)
        }
    ...
    fun resolveStructLiteral(
        ...
        if (typeSym == null || typeSym !is TypeSymbol) {
            errors.report(
                expr.loc,
                "Type '${structType.name}' is not defined",
                suggestion = "Declare it with 'type ${structType.name} { ... }' before using it",
            )
    ...
    private fun resolveFieldAccess(
        ...
        val suggestion =
            if (expr.fieldName == "length" &&
                (targetType == TypeRef.STRING || targetType.isArray)
            ) {
                ...
                errors.report(
                    ...
                    suggestion =
                        if (indexType ==
                            ...
                            "Use '.toInt()' to convert the index"
                        } else {
                            null
                        },
                )
    ...
    private fun resolveMethodCall(
        ...
        if (call.target is RawIdentifierExpr) {
            ...
            if (module != null) {
                ...
                if (importedFunc != null) {
                    ...
                    val tExpr =
                        TypedMethodCallExpr(
                            ...
                            TypedMethodCallExpr.Resolution.NAMESPACE,
                            callTarget,
                        )
                    validateArgs(
                        call.loc,
                ...
                errors.report(
                    call.loc,
                    "Namespace '$name' does not export a function named '${call.methodName}'",
                    suggestion = suggestion,
                )
        ...
        if (typeBoundMethod != null) {
            ...
            validateArgs(
                ...
                builtinSpecs(typeBoundMethod.params),
                typedArgs,
                scope,
            )
            return tExpr
        ...
        val ufcsCandidates =
            scope.allNamesInScope { sym ->
                sym is FuncSymbol &&
                    sym.params.isNotEmpty() &&
                    sym.params[0].type.isAssignableFrom(targetType)
            }
            ...
            when (expr.op) {
                            ...
                            val suggestion =
                                when {
                                    ...
                                    !left.type.isNumeric() ->
                                        ...
                                        DiagnosticHelpers.conversionHint(right.type, TypeRef.INT)
                                            ?: DiagnosticHelpers.conversionHint(right.type, TypeRef.DOUBLE)
                                }
                            errors.report(
                                expr.loc,
                                "Operator '${expr.op.symbol}' cannot be applied to '${left.type}' and '${right.type}'",
                                suggestion = suggestion,
                            )
                            TypeRef.JSON
                        }
                ...
                BinaryOp.LT, BinaryOp.LE, BinaryOp.GT, BinaryOp.GE -> {
                    if (!left.type.isNumeric() || !right.type.isNumeric()) {
                        errors.report(
                            expr.loc,
                            "'${expr.op.symbol}' requires two numeric operands (int or double), got '${left.type}' and '${right.type}'",
                ...
                BinaryOp.EQ, BinaryOp.NE -> {
                    if (!left.type.isComparable(right.type) &&
                        !(left is TypedNullLiteralExpr && right is TypedNullLiteralExpr)
                    ) {
                        errors.report(
                            expr.loc,
                            "Cannot compare '${left.type}' with '${right.type}' using '${expr.op.symbol}'",
                ...
                BinaryOp.BIT_AND, BinaryOp.BIT_OR, BinaryOp.BIT_XOR -> {
                    if (left.type != TypeRef.INT) {
                        errors.report(
                            expr.left.loc,
                            "Bitwise '${expr.op.symbol}' requires 'int' operands, got '${left.type}'",
                            suggestion =
                                if (left.type ==
                                    TypeRef.DOUBLE
                                ) {
                                    "Use '.toInt()' to convert"
                                } else {
                                    null
                                },
                        )
                    ...
                    if (right.type != TypeRef.INT) {
                        errors.report(
                            expr.right.loc,
                            "Bitwise '${expr.op.symbol}' requires 'int' operands, got '${right.type}'",
                            suggestion =
                                if (right.type ==
                                    TypeRef.DOUBLE
                                ) {
                ...
                BinaryOp.SHL, BinaryOp.SHR, BinaryOp.USHR -> {
                    if (left.type != TypeRef.INT) {
                        errors.report(
                            expr.left.loc,
                            "Shift '${expr.op.symbol}' requires 'int' operands, got '${left.type}'",
                            suggestion =
                                if (left.type ==
                                    TypeRef.DOUBLE
                                ) {
                    ...
                    if (right.type != TypeRef.INT) {
                        errors.report(
                            expr.right.loc,
                            "Shift '${expr.op.symbol}' requires 'int' operands, got '${right.type}'",
                            suggestion =
                                if (right.type ==
                                    TypeRef.DOUBLE
                                ) {
        ...
        for (i in args.indices) {
            ...
            if (param != null) {
                ...
                if (!expectedType.isAssignableFrom(argType)) {
                    errors.report(
                        args[i].loc,
                        "Argument ${i + 1} ('${param.name}') of '$funcName': expected '$expectedType', got '$argType'",
