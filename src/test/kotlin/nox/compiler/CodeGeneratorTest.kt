...
class CodeGeneratorTest : FunSpec({
    ...
    fun noxc(result: NoxCompiler.CompilationResult): String =
        result.disassembly ?: ""

    // Opcode selection

    test("intAddEmitsIADD") {
        val result = compileOk(
            """
    ...
    test("castStructArrayUsesSubOp1") {
        ...
        val hasArrayCast = result.compiledProgram!!.bytecode.any { inst ->
    ...
    test("voidReturnEmitsRET") {
        ...
        val retInstr = cp.bytecode.drop(doWorkMeta.entryPC)
    ...
    test("intReturnEmitsRETWithA") {
        ...
        val retInstr = cp.bytecode.drop(fnMeta.entryPC)
