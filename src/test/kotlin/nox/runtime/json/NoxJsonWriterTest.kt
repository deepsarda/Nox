...
class NoxJsonWriterTest : FunSpec({

    val writer = NoxJsonWriter()
    ...
    test("depth limit emits null for deeply nested structures") {
        val limited = NoxJsonWriter(maxDepth = 2)
    ...
    test("round-trip: parse then stringify complex nested structure") {
        val json = """{"users":[{"name":"Alice","active":true,"meta":null},{"name":"Bob","active":false,"score":3.14}]}"""
        val parsed = NoxJsonParser(json).parse()
